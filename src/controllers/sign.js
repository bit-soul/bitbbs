...
exports.signup = function (req, res, next) {
  ...
  // 验证信息的正确性
  if ([name, pass, rePass, email].some(function (item) { return item === ''; })) {
    ep.emit('prop_err', '信息不完整。');
    return;
  }
  if (name.length == 0) {
    ep.emit('prop_err', '用户名不能为空。');
    return;
  }
  ...
  User.getUsersByQuery({'$or': [
    ...
    tools.bhash(pass, ep.done(function (passhash) {
      ...
      User.newAndSave(name, passhash, email, null, false, function (err) {
        ...
        // 发送激活邮件
        mail.sendActiveMail(email, name, utility.md5(email + passhash + global.config.session_secret), id); //todo id 可能需要先查下
        res.render('sign/signup', {
...
exports.login = function (req, res, next) {
  ...
  User.getUserByMail(email, function (err, user) {
    ...
    tools.bcompare(pass, passhash, ep.done(function (bool) {
      ...
      if (!user.active) {
        // 重新发送激活邮件
        mail.sendActiveMail(user.email, user.name, utility.md5(user.email + passhash + global.config.session_secret), user._id);
        res.status(403);
...
exports.signout = function (req, res, next) {
  req.session.destroy();
  res.clearCookie(global.config.auth_cookie_name, { path: '/' });
  res.redirect('/');
};

exports.activeAccount = function (req, res, next) {
  var key  = validator.trim(req.query.key);
  var uid = validator.trim(req.query.uid);
  ...
  User.getUserById(uid, function (err, user) {
    ...
    if (!user) {
      return next(new Error('[ACTIVE_ACCOUNT] no such user: ' + uid));
    }
    var passhash = user.pass;
    if (!user || utility.md5(user.email + passhash + global.config.session_secret) !== key) {
...
exports.updateSearchPass = function (req, res, next) {
  ...
  User.getUserByMail(email, function (err, user) {
    ...
    user.save(function (err) {
      ...
      // 发送重置密码邮件
      mail.sendResetPassMail(email, retrieveKey, user._id);
...
exports.resetPass = function (req, res, next) {
  ...
  User.getUserById(uid, function (user) {
    ...
    return res.render('sign/reset', {uid: uid, key: key});
  });
};
...
exports.updatePass = function (req, res, next) {
  ...
  var uid  = validator.trim(req.body.uid) || '';

  var ep = new eventproxy();
  ep.fail(next);

  if (psw !== repsw) {
    return res.render('sign/reset', {uid: uid, key: key, error: '两次密码输入不一致。'});
  }
